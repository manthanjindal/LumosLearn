...
const Dashboard: React.FC = () => {
  ...
  // Derive stats from progress map
  const progressEntries = Object.values(progress || {});
  const lessonsCompletedCount = progressEntries.filter(p => p?.completed).length;
  const xpTotal = progressEntries.reduce((sum, p) => sum + (typeof p?.score === 'number' ? p.score : 0), 0);
  const level = Math.max(1, Math.floor(xpTotal / 100));
  // Simple 7-day activity streak heuristic
  const now = new Date();
  ...
  progressEntries.forEach(p => {
    if (p?.lastAccessed) {
      ...
      if (diffDays >= 0 && diffDays < 7) activeDays.add(d.toDateString());
    }
  });
  ...
  { id: 'ai_intro_quick', title: 'AI Basics: Quickstart', date: '2023-05-08', progress: 100 },
  ...
  const leaderboard = [
    ...
    { id: 5, name: user?.name || 'You', xp: xpTotal, level },
  ];
          ...
          <div className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-3xl font-bold text-[#145968]">{level}</p>
              </div>
            </div>
            <div className="mt-4">
              <div className="relative h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${(xpTotal % 100) || 100}%` }}
                ></div>
              </div>
              <p className="mt-1 text-xs text-gray-600 dark:text-gray-400">
                {xpTotal} XP ({(xpTotal % 100) || 100}/100 to next level)
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-3xl font-bold text-[#145968]">{streak}</p>
              </div>
            </div>
          ...
          <div className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-3xl font-bold text-[#145968]">{lessonsCompletedCount}</p>
              </div>
            </div>
            <div className="mt-4">
