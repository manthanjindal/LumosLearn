...
interface UserProgressContextType {
  progress: Progress;
  updateProgress: (lessonId: string, data: Partial<ProgressItem>) => void;
  markLessonComplete: (lessonId: string, score: number) => void;
  getLessonProgress: (lessonId: string) => ProgressItem;
...
export const UserProgressProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateProgress = (lessonId: string, data: Partial<ProgressItem>) => {
  ...
  const getLessonProgress = (lessonId: string): ProgressItem => {
    return progress[lessonId] || { completed: false, score: 0 };
  };

  const addNote = (lessonId: string, note: string) => {
    const currentProgress: ProgressItem = progress[lessonId] || { completed: false, score: 0, notes: [] };
